    ...
    <div class="py-8 max-w-5xl mx-auto sm:px-6 lg:px-8" x-data="aiSuggestShow()">
        <div class="bg-white shadow-sm rounded-md p-6 space-y-6">
            ...
            <div class="flex items-center gap-4 pt-2">
                ...
                @endif
                <button type="button" @click="open = !open; if(!ready && !loading){ fetchOrGenerate(false, false); }" class="ml-auto inline-flex items-center px-3 py-1.5 text-xs rounded border border-indigo-600 text-indigo-700 bg-white hover:bg-indigo-50">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 mr-1.5" viewBox="0 0 20 20" fill="currentColor"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l.563 1.737a1 1 0 00.95.69h1.826c.969 0 1.371 1.24.588 1.81l-1.477 1.073a1 1 0 00-.364 1.118l.563 1.737c.3.921-.755 1.688-1.538 1.118l-1.477-1.073a1 1 0 00-1.175 0l-1.477 1.073c-.783.57-1.838-.197-1.538-1.118l.563-1.737a1 1 0 00-.364-1.118L3.922 7.164c-.783-.57-.38-1.81.588-1.81h1.826a1 1 0 00.95-.69l.563-1.737z"/></svg>
            ...
            <div x-show="open" x-collapse x-cloak class="mt-3 border rounded-md bg-white shadow p-4 relative z-20">
                <div class="flex items-start justify-between">
                    <div>
                        ...
                        <p class="mt-1 text-[11px] text-slate-500">Automatically analyzed for this catch. Suggestions are cached—re-run to refresh.</p>
                    </div>
                ...
                <div class="mt-3">
                    ...
                    <template x-if="!error">
                        <div class="prose prose-sm max-w-none">
                            ...
                            <div x-show="!loading" x-cloak x-text="text" class="whitespace-pre-wrap text-sm text-slate-800"></div>
                        </div>
                    </template>
                </div>
                <div class="mt-4 flex items-center justify-between">
                    <div class="flex items-center gap-2">
                        <button type="button" @click="fetchOrGenerate(false, true)" class="inline-flex items-center px-3 py-1.5 text-xs rounded border border-slate-300 hover:bg-slate-50">Re-run analysis</button>
                        <button type="button" @click="open = false" class="inline-flex items-center px-3 py-1.5 text-xs rounded bg-gray-100 text-gray-800 hover:bg-gray-200">Hide</button>
                    </div>
    ...
    <script>
        function aiSuggestShow(){
            return {
                ...
                text: '',
                lastRunAt: null,
                autoAnalyze(){ this.fetchOrGenerate(false); },
                buildQuestion(){
                    return [
                ...
                async fetchOrGenerate(openAfter = true, force = false){
                    try{
                        ...
                        if (!force) {
                            const probe = await fetch("{{ route('ai.suggestions.catches.show', $catch) }}", { headers: { 'Accept': 'application/json' } });
                            const pj = await probe.json().catch(() => ({}));
                            if (probe.ok && pj.exists) {
                                this.text = (pj.content || '').replace(/^#+\s*/gm, '');
                                this.ready = true; this.lastRunAt = new Date(pj.updated_at || Date.now()).toLocaleString();
                                if (openAfter) this.open = true;
                                return;
                            }
                        }
                        const gen = await fetch("{{ route('ai.suggestions.catches.generate', $catch) }}" + (force ? '?force=1' : ''), {
                            method: 'POST',
                            headers: {
                        ...
                        if (!gen.ok || gj.error) {
                            this.error = gj.error || 'AI service is unavailable.'; this.ready = false; return;
                        }
